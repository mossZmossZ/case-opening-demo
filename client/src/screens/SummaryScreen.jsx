...
export default function SummaryScreen({ session, onPlayAgain }) {
  ...
  return (
    <div className="flex flex-col lg:h-screen lg:overflow-hidden bg-background text-on-surface font-body selection:bg-primary/20 animate-screen-in">
      ...
      <header className="fixed top-0 left-0 right-0 z-50 h-16 bg-white/95 backdrop-blur-xl border-b border-outline-variant shadow-sm">
        <div className="max-w-7xl mx-auto h-full px-8 flex items-center justify-between">
          <div className="flex items-center gap-6">
            <div className="text-lg font-black tracking-tight uppercase font-headline">
      ...
      <main className="flex-1 pt-16 lg:overflow-hidden lg:flex lg:flex-col">
        <div className="flex-1 min-h-0 max-w-7xl w-full mx-auto px-8 py-6 flex flex-col gap-5 lg:grid lg:grid-cols-[2fr_3fr] lg:gap-8">

          {/* ── Left — Best Reward Card ── */}
          {best ? (
            <div className="relative min-h-[360px] lg:min-h-0 animate-result-pop">
              {/* Ambient glow */}
              <div
          ...
          <div className="flex flex-col min-h-0 gap-4 animate-screen-in" style={{ animationDelay: '150ms' }}>
            ...
            <div className="flex-1 min-h-0 overflow-y-auto flex flex-col gap-3 pr-1">
              {results.map((r, i) => {
                ...
                return (
                  <div
                    key={i}
                    className="flex-shrink-0 relative flex items-center gap-4 p-4 bg-white border border-outline-variant shadow-sm transition-shadow hover:shadow-md"
                  >
                    ...
                    {/* Attempt number */}
                    <div className="flex-shrink-0 w-8 text-center pl-1">
                      <span className="font-mono text-xs font-bold text-on-surface-variant">#{i + 1}</span>
                    </div>

                    {/* Prize image or icon */}
                    <div
                      className="flex-shrink-0 w-14 h-14 bg-surface-container-low border flex items-center justify-center overflow-hidden"
                    ...
                    <div className="flex-shrink-0 flex flex-col items-end gap-1">
                      ...
                      {isBest && (
                        ...
                        >
                          <span className="material-symbols-outlined text-[10px]" style={{ fontVariationSettings: "'FILL' 1" }}>military_tech</span>
                          Best
                        </span>
                      )}
                    </div>
            ...
            <div className="flex-shrink-0 pt-3 border-t border-outline-variant">
              <button
                onClick={onPlayAgain}
                className="w-full h-12 bg-primary text-on-primary font-headline font-bold text-sm uppercase tracking-wider flex items-center justify-center gap-2 hover:bg-primary-fixed active:scale-[0.98] transition-all shadow-[0_2px_12px_rgba(224,96,32,0.25)] hover:shadow-[0_4px_20px_rgba(224,96,32,0.4)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2"
              >
